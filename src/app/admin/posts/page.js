...
export default function AdminPostsPage() {
  ...
  if (loading) return <div className="text-slate-400">{t("common.loading", "Loading posts...")}</div>;

  return (
    <div>
      <h1 className="text-2xl font-bold text-white mb-6">
        {t("admin.carPostsTitle", "Manage Car Posts")}
      </h1>
          ...
          <div className="bg-navy-900 border border-white/10 rounded-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
            <div className="p-6 border-b border-white/10 flex justify-between items-center sticky top-0 bg-navy-900 z-10">
              <h2 className="text-xl font-bold text-white">
                {t("admin.postDetailsTitle", "Post Details")}
              </h2>
              <button 
            ...
            <div className="p-6 space-y-6">
              <div className="flex justify-between items-start">
                <div>
                  ...
                  <p className="text-sm text-slate-400">
                    {selectedPost.year} • {t(`conditions.${selectedPost.condition}`, selectedPost.condition)}
                  </p>
                </div>
              ...
              <div className="text-sm text-slate-300 space-y-1">
                <p><strong>{t("admin.descLabel", "Description")}:</strong> {selectedPost.description || "N/A"}</p>
                <p><strong>{t("admin.locationLabel", "Location")}:</strong> {selectedPost.locationAddress}</p>
                <p><strong>{t("admin.userLabel", "Customer")}:</strong> {selectedPost.userId?.name} ({selectedPost.userId?.phone})</p>
              </div>

              {/* Actions */}
              <div className="border-t border-white/10 pt-6 space-y-4">
                <h4 className="text-sm font-semibold text-white">
                  {t("admin.adminActions", "Admin Actions")}
                </h4>
                ...
                <form onSubmit={sendOffer} className="flex gap-3 pt-4">
                  <input
                    type="number"
                    placeholder={t("admin.offerAmountPlaceholder", "Offer Amount (AED)")}
                    value={offerPrice}
                    onChange={(e) => setOfferPrice(e.target.value)}
                    className="flex-1 px-4 py-2 rounded-lg bg-navy-950 border border-white/10 text-white text-sm"
                  />
                  <button type="submit" className="px-4 py-2 rounded-lg bg-accent text-white text-sm font-semibold cursor-pointer">
                    {t("admin.sendOfferBtn", "Send Offer")}
                  </button>
                </form>
              </div>
            </div>
      ...
      <div className="bg-navy-900/60 border border-white/5 rounded-2xl overflow-hidden">
        <table className="w-full text-left text-sm text-slate-300">
          <thead className="bg-navy-950 border-b border-white/5 text-xs uppercase text-slate-500">
            <tr>
              <th className="px-6 py-4">{t("admin.tableCar", "Car")}</th>
              <th className="px-6 py-4">{t("admin.tableStatus", "Status")}</th>
              <th className="px-6 py-4">{t("admin.tableOffer", "Offer Price")}</th>
              <th className="px-6 py-4">{t("admin.tableDate", "Date")}</th>
              <th className="px-6 py-4">{t("admin.tableAction", "Action")}</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-white/5">
            {posts.map((post) => (
              <tr key={post._id} className="hover:bg-white/[0.02] transition-colors">
                ...
                <td className="px-6 py-4">
                  {post.offerPrice ? `${post.offerPrice.toLocaleString()} ${t("common.aed", "AED")}` : "-"}
                </td>
                ...
                <td className="px-6 py-4">
                  <button
                    ...
                    className="text-accent hover:text-white transition-colors font-medium text-xs bg-accent/10 px-3 py-1.5 rounded-lg cursor-pointer"
                  >
            ...
            {posts.length === 0 && (
              <tr>
                <td colSpan="5" className="px-6 py-8 text-center text-slate-500">
                  {t("common.noData", "No posts found.")}
                </td>
              </tr>
            )}
          </tbody>
